...
void handle_interrupt(long cause)
{
 ...
 if (cause==IRQ_M_EXT)
 {
 	//hid_send_string("Coucou");
 	external_interrupt();  
 }
 //hid_send_string("Clearing");
 mip = read_csr(mip);
 //snprintf(code, sizeof(code), "mip=%x\n", mip);
 //hid_send_string(code);
...
void test_return()
{
	...
	hid_send_string("Sortie");
	
	mie = read_csr(mie);
 	snprintf(code, sizeof(code), "mie=%x\n", mie);
 	hid_send_string(code);
	return;
}
...
void init_plic(void)
{
  ...
  for (i = 1; i <= 64; i++)
    {
      plic[0x800+i/32] |= 1<<(i&31); // Starts from 0x0C00_2000, which represents Hart 0 M-Mode Interrupt Enable.
    }
  plic[0x80000] = 0; // Write 0 to address 0x0C20_0000 ==> Threshold = 0 
  for (i = 0; i < 5; i++)
    printf("%x: %x\n", i, plic[i]);  // Print the Priority of source 0, (which doesn't exist)
    								// Followed by the Priorities of the first four Interrupt Sources.
  									// prints 0 1 1 1 ...
  for (i = 0x800; i < 0x804; i++)
    printf("%x: %x\n", i, plic[i]); // Print the Interrupt enables.
    								// 0x1E ==> 1st bit corresponds to source 0, which does exist
    								//          4 bits set to '1', which represent the 4 interrupt sources.
}
...
void _init(int cid, int nc)
{
  ...
  printf("clearing    bsslen = %d   Bytes\n", bsslen);
  ...
  */
  ...
  // A second plic init is made in the main function of the testInterrupt.c file
  // init_plic();
